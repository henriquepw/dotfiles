{ ... }:
{
  flake.nixosModules.kde =
    { pkgs, lib, ... }:
    {
      # Toolkits na sessão Wayland (Qt/GTK/Clutter)
      ...
      # Desktop Plasma 6 (ex-hosts/citadel/desktop.nix)
      ...
      home-manager.sharedModules = [
        (
          ...
          }:
          {
            ...
            home.file.".XCompose".source = link "${repoRoot}/dendritic/features/kde/config/XCompose";
            ...
            home.activation.plasma-panel = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
              ...
              if [ ! -f "$_PANEL" ]; then
                cp "${repoRoot}/dendritic/features/kde/config/plasma-org.kde.plasma.desktop-appletsrc" "$_PANEL"
                chmod 644 "$_PANEL"
              fi
            '';
